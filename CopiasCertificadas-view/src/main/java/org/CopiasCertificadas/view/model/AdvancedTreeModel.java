...
public class AdvancedTreeModel extends DefaultTreeModel<Tramites> {
	private static final long serialVersionUID = -5513180500300189445L;
	
	DefaultTreeNode<Tramites> _root;

	public AdvancedTreeModel(TramiteNotaTreeNode contactTreeNode) {
	 ...
	 */
	public void remove(DefaultTreeNode<Tramites> parent, int indexFrom, int indexTo) throws IndexOutOfBoundsException {
		DefaultTreeNode<Tramites> stn = parent;
	...
	public void remove(DefaultTreeNode<Tramites> target) throws IndexOutOfBoundsException {
		int index = 0;
		DefaultTreeNode<Tramites> parent = null;
	 ...
	 *             - indexFrom < 0 or indexTo > number of parent's children
	 */
	public void insert(DefaultTreeNode<Tramites> parent, int indexFrom, int indexTo, DefaultTreeNode<Tramites>[] newNodes)
			throws IndexOutOfBoundsException {
		DefaultTreeNode<Tramites> stn = parent;
	 ...
	 */
	public void add(DefaultTreeNode<Tramites> parent, DefaultTreeNode<Tramites>[] newNodes) {
		DefaultTreeNode<Tramites> stn = (DefaultTreeNode<Tramites>) parent;
	...
	private DefaultTreeNode<Tramites> dfSearchParent(DefaultTreeNode<Tramites> node, DefaultTreeNode<Tramites> target) {
		if (node.getChildren() != null && node.getChildren().contains(target)) {
			return node;
		} else {
			int size = getChildCount(node);
			for (int i = 0; i < size; i++) {
				DefaultTreeNode<Tramites> parent = dfSearchParent((DefaultTreeNode<Tramites>) getChild(node, i), target);
				if (parent != null) {
					return parent;
				}
			}
