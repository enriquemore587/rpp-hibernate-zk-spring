...
public class PagosVentanillaPFVM implements Serializable {
	...
	public void sendPago(@ContextParam(ContextType.VIEW) Component view) {
		if (validar()) {
			...
			BindUtils.postGlobalCommand(null, null, "addPagoVentanillaPF", map);
	...
	@GlobalCommand
	@NotifyChange("pagos")
	public void updatePagoVentanillaPF(@BindingParam("pago") Pagos pagos) {
		this.pagos = pagos;
		this.editar = true;
	}
